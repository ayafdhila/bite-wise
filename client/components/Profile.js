...
const chartConfig = {
    ...
    decimalPlaces: 0,
    color: (opacity = 1) => `rgba(46, 74, 50, ${opacity})`,
    labelColor: (opacity = 1) => `rgba(46, 74, 50, ${opacity})`,
    style: { borderRadius: 16, },
    propsForDots: { r: "0", strokeWidth: "0", fill: PALETTE.darkGreen },
...
export default function Profile() {
    ...
    const USER_PROFILE_ENDPOINT = uid ? `${API_BASE_URL}/user/profile/${uid}` : null;
    const CALORIE_HISTORY_ENDPOINT = `${API_BASE_URL}/profile/calorie-history`;
    ...
    const itemWidth = 25; // CHANGED to 25 to match SettingProfile's rulerItem width
    const minWeight = 35; const maxWeight = 230;
    ...
    }, [uid, getIdToken, USER_PROFILE_ENDPOINT]);
    ...
    }, [uid, getIdToken, CALORIE_HISTORY_ENDPOINT]);
    ...
    useEffect(() => {
        if (uid && !isProfileLoading) {
            fetchCalorieHistory(selectedChartPeriod);
        }
    }, [selectedChartPeriod, uid, isProfileLoading, fetchCalorieHistory]);
    ...
    const handleLogNewWeight = useCallback(async () => {
        ...
        try {
            ...
            const logWeightResponse = await axios.post(LOG_WEIGHT_ENDPOINT,
                { weight: newWeight, date: dateString },
            ...
            const currentTargetWeightState = parseFloat(goalWeight);
            const currentUserGoalTypeStateLower = (userGoalType || '').toLowerCase();
            ...
            if (currentUserGoalTypeStateLower === 'losing weight' && !isNaN(currentTargetWeightState) && currentTargetWeightState > 0) {
                ...
                const maintenanceTolerance = 1.0;
            ...
            if (goalMet) {
                ...
                if (!achievements?.goalWeightReached) {
    ...
    const handleRulerScroll = useCallback((event) => {
        ...
        if (numbersRuler[index]) {
            setRulerSelectedValue(numbersRuler[index]);
        }
    }, [itemWidth, numbersRuler, setRulerSelectedValue]); // Added setRulerSelectedValue
            ...
            <ScrollView contentContainerStyle={styles.scrollContentContainer} showsVerticalScrollIndicator={false}>
                ...
                <View style={styles.rulerSectionContainer}>
                    ...
                    <View style={styles.rulerVisualContainer}>
                        <FlatList
                            ...
                            disableIntervalMomentum={true} // ADDED THIS PROP
                            keyExtractor={(item) => `ruler-tick-${item}`}
                ...
                <View style={styles.bmiDisplayContainer}>
                    <View style={styles.bmiDisplayBox}>
                        ...
                        <Text style={styles.bmiValueDisplay}>{bmiValue}</Text>
                    </View>
                    <View style={styles.bmiDisplayBox}>
                        <Text style={styles.bmiLabel}>Category</Text>
                        <Text style={styles.bmiCategoryDisplay}>{bmiCategory}</Text>
                    </View>
                </View>
            </ScrollView>
            <TabNavigation />
                        ...
                        {popupPlanData && (
                            <View style={styles.planDetailsContainer}>
                                <View style={styles.planDetailRow}>
                                    <Text style={styles.planDetailLabel}>Calories:</Text>
                                    <Text style={styles.planDetailValue}>{popupPlanData.calories?.toFixed(0)} kcal</Text>
                                </View>
                                <View style={styles.planDetailRow}>
                                    <Text style={styles.planDetailLabel}>Protein:</Text>
                                    <Text style={styles.planDetailValue}>{popupPlanData.protein?.toFixed(0)} g</Text>
                                </View>
                                <View style={styles.planDetailRow}>
                                    <Text style={styles.planDetailLabel}>Carbs:</Text>
                                    <Text style={styles.planDetailValue}>{popupPlanData.carbs?.toFixed(0)} g</Text>
                                </View>
                                <View style={styles.planDetailRow}>
                                    <Text style={styles.planDetailLabel}>Fat:</Text>
                                    <Text style={styles.planDetailValue}>{popupPlanData.fat?.toFixed(0)} g</Text>
                                </View>
                                <View style={styles.planDetailRow}>
                                    <Text style={styles.planDetailLabel}>Fiber:</Text>
                                    <Text style={styles.planDetailValue}>{popupPlanData.fiberGoal}</Text>
                                </View>
                            </View>
                        )}
...
const styles = StyleSheet.create({
    ...
    errorDisplayContainer: { flex: 1, justifyContent: 'center', alignItems: 'center', padding: 20 },
    ...
    weightBarContainer: { flexDirection: 'row', elevation: 2, backgroundColor: PALETTE.white },
    weightBarItem: { flex: 1, alignItems: 'center', height: 70, justifyContent: 'center', paddingVertical: 10 },
    ...
    scrollContentContainer: { alignItems: 'center', paddingHorizontal: 15, paddingBottom: 100 },
    sectionTitleProfile: { marginVertical: 25, fontSize: 20, fontFamily: "Quicksand_700Bold", color: PALETTE.darkGreen, textAlign: 'center' },
    ...
    periodToggleText: { fontSize: 14, fontFamily: "Quicksand_600SemiBold" },
    periodToggleTextActive: { color: PALETTE.darkGreen, fontFamily: "Quicksand_700Bold" },
    ...
    chartStyle: { borderRadius: 10, marginVertical: 8 },
    ...
    buttonDisabled: { backgroundColor: PALETTE.grey, opacity: 0.7 },
    ...
    detailText: { fontSize: 16, fontFamily: "Quicksand_500Medium", color: PALETTE.darkGrey, marginBottom: 8, textAlign: 'left', width: '100%' },
